...
def graph_search(problem, fringe):
    ...
    while fringe:
        node = fringe.pop()
        exp_nodes = exp_nodes + 1
        if problem.goal_test(node.state):
            print("Nr of expanded nodes: ", exp_nodes)
            return node
...
def babh(problem):
    """Search through branch and bound with sub-estimation"""
    return graph_search(problem, Babhg(problem))

def bab(problem):
    """Search through branch and bound"""
