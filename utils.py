...
class Babhg(Queue):
    """FIFO Queue - Branch and bound with heuristic"""
    ...
    def extend(self, items):
        self.A.extend(items)
        list.sort(self.A, reverse=True, key=self.heuristic_plus_path_cost)

    def heuristic_plus_path_cost(self, node):
...
class Babg(Queue):
    ...
    def extend(self, items):
        self.A.extend(items)
        list.sort(self.A, reverse=True, key=operator.attrgetter("path_cost"))
